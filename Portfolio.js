...
import { getFirestore, collection, addDoc, deleteDoc, doc, onSnapshot, query, orderBy, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.9.0/firebase-firestore.js";

const firebaseConfig = {
    apiKey: "AIzaSyBtIx6Ue6mYRbqe4JxygtLWzP-J8OOB6_0",
    authDomain: "portfolio-comment-781e9.firebaseapp.com",
    projectId: "portfolio-comment-781e9",
    storageBucket: "portfolio-comment-781e9.firebasestorage.app",
    messagingSenderId: "6997309301",
    appId: "1:6997309301:web:78f5928d2c20fb748f453c"
};
...
// Rating buttons active state
const ratingBtns = document.querySelectorAll('.rating-row button');
ratingBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ratingBtns.forEach(b => b.classList.remove('activeBTN'));
...
function renderComment(docSnap) {
    ...
    const rating = escapeHTML(data.rating || 'None');
    const message = escapeHTML(data.message || '');
    const color = data.color || '#3498db';
    const time = formatTime(data.createdAt);

    return `
        <div class="comment-card" data-id="${docSnap.id}">
            <div class="profile-user">
                <img style="background: ${color};" src="portfolioImg-about/user img.png" alt="user">
            </div>
...
let lastPostTime = 0;
const POST_COOLDOWN = 30000; // 30 วินาที
...
function postComment() {
    ...
    postBtn.addEventListener('click', async () => {
        ...
        if (now - lastPostTime < POST_COOLDOWN) {
            const wait = Math.ceil((POST_COOLDOWN - (now - lastPostTime)) / 1000);
            alert(`กรุณารอ ${wait} วินาทีก่อนโพสต์ใหม่`);
            return;
        }
        ...
        if (!nameValue || !msgValue) {
            alert("กรุณาใส่ชื่อและข้อความให้ครบถ้วน");
            return;
        }
        ...
        try {
            const btnLabel = postBtn.querySelector('.btn-label');
            if (btnLabel) btnLabel.textContent = 'กำลังโพสต์...';
            postBtn.style.pointerEvents = 'none';

            // ลบข้อความออกทันทีเพื่อให้ลื่นไหล
            nameInput.value = '';
            msgInput.value = '';
            ...
            if (btnLabel) btnLabel.textContent = 'Post Comment';
            postBtn.style.pointerEvents = 'auto';

        } catch (error) {
            console.error("Error adding document: ", error);

            // คืนค่ากลับมาหากโพสต์ไม่สำเร็จ
            nameInput.value = nameValue;
            msgInput.value = msgValue;
            nameInput.disabled = false;
            msgInput.disabled = false;

            alert("เกิดข้อผิดพลาดในการโพสต์ หากยังไม่ได้เปิด Firestore หรือตั้ง Rules ผิด กรุณาตรวจสอบ");
            const btnLabel = postBtn.querySelector('.btn-label');
            if (btnLabel) btnLabel.textContent = 'Post Comment';
            postBtn.style.pointerEvents = 'auto';
        }
...
function loadComments() {
    const listContainer = document.querySelector('.js-comments-list');

    // Loading state
    listContainer.innerHTML = `<div class="comments-loading">
        <span class="loading-spinner"></span> กำลังโหลดคอมเม้น...
    </div>`;
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            commentsHTML += renderComment(docSnap);
        });

        listContainer.innerHTML = commentsHTML || '<p class="no-comments">ยังไม่มีคอมเม้น เป็นคนแรกได้เลย! 👋</p>';
